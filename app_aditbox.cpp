...
{
#if 0
    static constexpr buttons_t buttonmap2[] = {
        {5, 105, 0, "", 0xf},
        {90, 105, 0, "", 0xf},
        {175, 105, 0, "", 0xf},
        {5, 155, 0, "", 0xf},
    ...
    {
        _buttons.at(i).pos(buttonmap2[i].x, buttonmap2[i].y);
        _buttons.at(i).id(buttonmap2[i].id);
        _buttons.at(i).size(60, 40);
        _buttons.at(i).label(buttonmap2[i].label);
        _buttons.at(i).fill(buttonmap2[i].color);
    }
#endif
}
...
int main()
{
    ...
    while (true)
    {
        ...
        if (p.z > 200)
        {
            uint16_t ret = pincode.dispatch(p);
#if 0
            switch (ret)
            {
            case ID_FRAME_MENU:
